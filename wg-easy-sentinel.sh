...
send_notification() {
  ...
  if [[ -z "$PUSHOVER_APP_TOKEN" || -z "$PUSHOVER_USER_KEY" ]]; then
      log "⚠️ Warning: Pushover credentials not set. 🔕 Skipping notification: '${title}' ⚠️"
...
build_friendly_names() {
    ...
    if [[ "$line" =~ ^#\ Client:\ (.*)\ \(.+\)$ ]]; then
      ...
      if [[ -n "$current_friendly" && -n "$pubkey" ]]; then
        ...
        log "🐛 Mapped PublicKey $pubkey to friendly name '$current_friendly' 🐛" # Debug log
         ...
         log "⚠️ Warning: Found PublicKey '$pubkey' without preceding friendly name comment. ⚠️"
      fi
  ...
  if [[ ${#friendly_names[@]} -eq 0 ]]; then
      log "⚠️ Warning: No friendly names found in the WireGuard config. ⚠️"
  fi
}
...
fetch_handshake_info() {
  ...
  echo "$output" | grep -E '^[^\s]+\s+[^\s]+\s+[0-9]+$' || {
      log "⚠️ No valid handshake lines found in 'wg show' output. ⚠️"
...
load_state() {
  ...
  if [[ ! -f "$STATE_FILE" ]]; then
    log "ℹ️ State file '$STATE_FILE' not found. Starting fresh. ℹ️"
    return 0
  fi

  log "ℹ️ Loading state from $STATE_FILE ℹ️"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$type" == "handshake" && -n "$key" && "$value" =~ ^[0-9]+$ ]]; then
      ...
      log "⚠️ Warning: Skipping malformed line in state file: $line ⚠️"
    fi
  done < "$STATE_FILE"
  log "ℹ️ Loaded state: ${#connected_peers[@]} previously connected peers, ${#last_handshakes[@]} known handshakes. ℹ️"
}

# Save state from associative arrays to file (atomically)
save_state() {
  local temp_state_file="${STATE_FILE}.tmp"
  log "ℹ️ Saving state to $STATE_FILE ℹ️"
...
if [[ -z "$PUSHOVER_APP_TOKEN" || -z "$PUSHOVER_USER_KEY" ]]; then
  log "⚠️ Warning: PUSHOVER_APP_TOKEN or PUSHOVER_USER_KEY is not set. Notifications will be skipped. ⚠️"
...
log "ℹ️ Building friendly names map... ℹ️"
...
log "ℹ️ Fetching current handshake info... ℹ️"
...
if [[ $fetch_status -ne 0 ]]; then
    ...
    log "ℹ️ No active peers found in current handshake info. ℹ️"
...
while IFS= read -r line; do
  ...
  if [[ $time_since_handshake -lt $TIMEOUT_THRESHOLD ]]; then
    ...
    if [[ -z "${connected_peers[$peer]:-}" ]]; then
      ...
      send_notification "🟢 Peer Connected" "👤 Peer $peer_display is now online." || log "⚠️ Warning: Failed to send connection notification for $peer_display ⚠️"
...
for peer in "${!connected_peers[@]}"; do # Iterate peers connected *last time*
  ...
  if [[ -z "${current_connected_peers[$peer]:-}" ]]; then
    ...
    send_notification "🔴 Peer Disconnected" "👤 Peer $peer_display appears to be offline (Last handshake: ${time_since_last_seen}s ago)." || log "⚠️ Warning: Failed to send disconnection notification for $peer_display ⚠️"
